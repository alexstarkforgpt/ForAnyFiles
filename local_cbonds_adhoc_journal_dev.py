...
SOURCE_CODE_CBONDS = 12
EXLOG_GROUP_CBONDS = 12
PAGE_LIMIT = 1000
CBONDS_TIMEOUT_SECONDS = 600
MAX_METHOD_RETRIES = 25
RETRY_DELAY_SECONDS = 120
PAGE_DELAY_SECONDS = 2.1
FATAL_CBONDS_ERR_NOS = frozenset({500100})

# False = как старый скрипт: ошибка одной записи не валит весь прогон.
# True = в конце main() будет ненулевой exit code при failed > 0.
FAIL_ON_METHOD_ERROR = False
...
def resolve_runtime_config() -> RuntimeConfig:
    ...
    port_raw = _env_first("MSSQL_PORT", default=str(MSSQL_PORT))
...
def validate_runtime_config(cfg: RuntimeConfig) -> None:
    ...
    if not cfg.cbonds_login:
        missing.append("CBONDS_LOGIN (env или константа в CONFIG)")
    ...
    if not cfg.mssql_trusted_connection:
...
def log_runtime_config(cfg: RuntimeConfig) -> None:
    ...
    auth_mode = "Trusted_Connection" if cfg.mssql_trusted_connection else f"UID={cfg.mssql_user}"
...
INIT_WITHOUT_UPDATES_METHODS = STATIC_METHODS_CBONDS | {
    "cbonds_get_stocks_dividends_v2",
    "cbonds_get_flow_new",
    "cbonds_get_stocks_beta_coefficient",
    "cbonds_get_etf_share_classes_quotes",
...
class CbondsJournalError(Exception):
    """Ошибка обработки одной записи журнала Cbonds."""


class CbondsApiError(CbondsJournalError):
    """Cbonds вернул JSON-блок error или некорректный ответ."""


class CbondsApiFatalError(CbondsApiError):
...
class JournalRequest:
    ...
    schedule_source_id: int | None
    his_from: datetime | None
    his_to: datetime | None
    his_master: int | None
    fix_master: int | None
    is_err: int | None
    err_desc: str | None
    comment: str | None
    updates_from: datetime | None
    updates_to: datetime | None
    chars: int | None
    lines: int | None
    kbytes: int | None
...
def connect_mssql():
    ...
    cfg = _require_cfg()
    driver = cfg.mssql_odbc_driver
    conn_str = (
        f"DRIVER={{{driver}}};"
        f"SERVER={cfg.mssql_host},{cfg.mssql_port};"
        f"DATABASE={cfg.mssql_database};"
    )
    if cfg.mssql_trusted_connection:
        if cfg.mssql_user:
            conn_str += f"UID={cfg.mssql_user};"
        conn_str += "Trusted_Connection=yes;"
    else:
        conn_str += f"UID={cfg.mssql_user};PWD={cfg.mssql_password};"
...
def cbonds_auth() -> dict[str, str]:
    ...
    cfg = _require_cfg()
    return {"login": cfg.cbonds_login, "password": cfg.cbonds_password}
...
def fetch_pending_requests(conn: pyodbc.Connection) -> list[JournalRequest]:
    ...
    requests_to_process = [JournalRequest.from_row(row) for row in rows]
...
def write_success_to_sql(conn: pyodbc.Connection, process_name: str, total: int) -> None:
    ...
    message = f"{process_name}, Total: {total}"
    sql = "EXEC BCS_BIZ.BBG.spNSD_CBonds_ExLog_DEV ?, ?, ?"
    execute_no_result(conn, sql, (EXLOG_GROUP_CBONDS, message, datetime.now()))
...
def write_error_to_sql(conn: pyodbc.Connection, process_name: str, error_text: str) -> None:
    ...
    sql = "EXEC BCS_BIZ.BBG.spNSD_CBonds_ExLog_DEV ?, ?, ?, ?"
    execute_no_result(conn, sql, (EXLOG_GROUP_CBONDS, process_name, datetime.now(), error_text))
...
def mark_request_completed(
    ...
    if response_xml is None:
        execute_no_result(
            ...
            WHERE Request_id = ?
            """,
            (datetime.now(), comment, request_id),
        )
    else:
        execute_no_result(
            conn,
            ...
            WHERE Request_id = ?
            """,
            (datetime.now(), comment, response_xml, request_id),
        )
...
def create_child_request(
    ...
    child_request_id = next(iter(rows[0].values()))
...
def method_group(code: str) -> int:
    ...
    if code in STATIC_METHODS_CBONDS or code in PRICE_METHODS_CBONDS:
        return 1
...
def trim_empty_values(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        return {k: trim_empty_values(v) for k, v in value.items() if v is not None and v != ""}
    if isinstance(value, list):
        return [trim_empty_values(item) for item in value]
    return value
...
def response_to_sql_xml_text(response: dict[str, Any], trim_empty: bool) -> str:
    ...
    """
    payload = trim_empty_values(response) if trim_empty else response
...
def parse_request_xml(request_xml: str | None) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    cleaned = request_xml.strip()
    if cleaned.startswith("<!--"):
        cleaned = cleaned.removeprefix("<!--").removesuffix("-->").strip()

    if not cleaned or cleaned.lower() == "no xml":
        return [], []
...
def sorting_for_method(code: str) -> list[dict[str, str]]:
    ...
    """
    if code == "cbonds_get_tradings_new":
        return [{"field": "created_at", "order": "asc"}]
    if code == "cbonds_get_index_value_new":
        return [{"field": "date", "order": "asc"}, {"field": "updated_at", "order": "asc"}]
...
def post_cbonds_page(
    ...
    response = session.post(
        request.data_url,
        json=body,
        proxies=_require_cfg().proxies,
        timeout=CBONDS_TIMEOUT_SECONDS,
    )
    ...
    if "error" in data:
        ...
        err_str = err.get("err_str") or str(err)
        if err_no in FATAL_CBONDS_ERR_NOS:
...
def get_cbonds_schema_text(session: requests.Session, auth: dict[str, str], request: JournalRequest) -> str:
    ...
    if request.request_code in (230, 233):
        params["lang"] = "rus"
    separator = "&" if "?" in request.data_url else "?"
    url = request.data_url + separator + urlencode(params)
    log.info("GET Cbonds schema method=%s request_id=%s url=%s", request.code, request.request_id, request.data_url)
    response = session.get(url, proxies=_require_cfg().proxies, timeout=CBONDS_TIMEOUT_SECONDS)
    response.raise_for_status()
...
def request_windows(
    ...
    защищённым от `None`.
    """
    his_from = format_sql_datetime(request.his_from)
    his_to = format_sql_datetime(request.his_to)

    if request_type_code == 3 and request.code in INIT_WITHOUT_UPDATES_METHODS:
        updates_from = None
        updates_to = None
    else:
    ...
    return his_from, his_to, updates_from, updates_to
...
def save_cbonds_page(
    ...
    """
    comment = f"{request.comment}:  пп 0 из {total}" if total > 0 else f"{request.comment}: пп 0 из {total}"

    if total <= PAGE_LIMIT or offset == 0:
        log.info(
        ...
        mark_request_completed(conn, request.request_id, comment, response_xml)
        return
    ...
    child_request_id = create_child_request(
        ...
        his_from=his_from if group == 1 else None,
...
def process_json_method(
    ...
    fields: list[dict[str, Any]] = []
    if group == 1:
        filters, fields = parse_request_xml(request.request_xml)

    sorting = sorting_for_method(request.code)
    his_from, his_to, updates_from, updates_to = request_windows(request, type_request)
    ...
    first_response = post_cbonds_page(session, request, first_body)
    total = get_total(first_response)
    log.info(
    ...
    for offset in range(0, total, PAGE_LIMIT):
        if offset == 0:
            ...
            page_response = post_cbonds_page(session, request, page_body)

        response_xml = response_to_sql_xml_text(page_response, trim_empty=(group == 1))
        save_cbonds_page(
            conn=conn,
            request=request,
            type_request=type_request,
...
def process_schema_method(
    ...
    schema_text = get_cbonds_schema_text(session, auth, request)
...
def process_one_request(
    ...
    """
    request_type_code = request_type_to_code(request.comment)
    group = method_group(request.code)
    last_error = ""
    ...
    for attempt in range(1, MAX_METHOD_RETRIES + 1):
        try:
            if group == 3:
                ...
                total = process_json_method(conn, session, auth, request, group, request_type_code)

            write_success_to_sql(conn, request.process_name, total)
            log.info(
            ...
            error_text = f"{exc.err_no}: {exc.err_str}"
            log.error(
...
def run_cbonds_adhoc_journal() -> dict[str, int]:
    ...
    begin = datetime.now()
    date_request = datetime.now() - timedelta(hours=5)
    ...
    stats = {"found": 0, "success": 0, "failed": 0}
    auth = cbonds_auth()

    with connect_mssql() as conn:
        pending_requests = fetch_pending_requests(conn)
        stats["found"] = len(pending_requests)

        with requests.Session() as session:
            for request in pending_requests:
                ok = process_one_request(conn, session, auth, request)
                if ok:
                    stats["success"] += 1
                else:
                    stats["failed"] += 1
    ...
    if FAIL_ON_METHOD_ERROR and stats["failed"]:
...
def main() -> int:
    ...
    log_runtime_config(cfg)
    _CFG = cfg

    try:
        stats = run_cbonds_adhoc_journal()
    finally:
        _CFG = None

    log.info("Local DEV AdHoc run finished with stats=%s", stats)
    if FAIL_ON_METHOD_ERROR and stats.get("failed"):
        return 1
    return 0
